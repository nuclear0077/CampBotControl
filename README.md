...
## About
Управление для camp бота образовательной платформы.

Реализовано наполнение базы данных через веб интерфейс и API для получения информации из базы данных.
...
Отправляем POST запрос на адрес http://localhost:8000/api-token-auth/ тело сообщения JSON формата:
```
{
...
Значения могут дублироваться, так как пересекаются в типах образования.
...
[
    {
        "id": 1,
        "name": "Подробное описание профиля, данное поле переименовано в сериализаторе для удобства парсинга JSON"
    }
]
```
